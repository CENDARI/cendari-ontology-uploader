<!DOCTYPE HTML>
<%@ page import="java.io.*, javax.servlet.*, javax.servlet.http.*, org.cendari.ontology.utility.Utility, java.util.*, org.json.simple.JSONArray, org.json.simple.JSONObject" %>
<html lang="en">
<head>
...
<body>
	...
	<div class="container">
		...
		<ul class="breadcrumb" style="background-color:white">
		  ...
		  <li class="active"><%=session.getAttribute("dataspaceTitle") %></li>
		</ul>
		<hr>
			...
			<table class="table table-striped table-hover ">
			  	<thead>
			    	<tr>
			      		...
			      		<th class="col-md-3">&nbsp;&nbsp;Dataset Title</th>
			  	...
			  	<tbody>
			  		<%	
						...
						if (datasetArray != null) {
							...
							for (int count = 0; count < datasetArray.size(); count++) {
						    	JSONObject dataset = (JSONObject)datasetArray.get(count);
					%>
					<tr>
			      		<td><%=count+1%></td>
			      		<td>
							<form action="ListResources.do" method="GET">
								<input type="hidden" name="datasetId" value="<%= dataset.get("id")%>">
								<input type="hidden" name="datasetTitle" value="<%=dataset.get("title")%>">
								<input type="hidden" name="resources" value="<%=dataset.get("resources") %>">
								<button type="submit" class="btn-link"><%=dataset.get("title") %></button>
							</form>
						</td>
